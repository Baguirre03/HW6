 ...
 * discussed in class).
 *
 * To allow the application (aka, user of this class) to have very basic
 * priority
 * management, the class does provide a method, changePriority(), such that you
 * can change a priority of an element already on the queue. The invoker will
 * need
 * to know the node's element to use this (aka, have a handle to it), which is
 * returned from the method which adds the element to the queue. This is for
 * very
 ...
 *
 * A more sophisticated approach would be to implement a weighted priority
 * scheme
 * that optionally allows aging and/or fair scheduling techniques to ensure
 * there
 * is no starvation. This is simple to do with this class. It is accomplished,
 * by
 ...
 * ArrayList<E> toArray - returns an array consisting of the element values in
 * the
 * queue.
 * void printPriorityQueue - This method is for debugging purposes, it will
 * print
 * out the min heap
 *
 *
...
class PriorityQueue<E, P> {
    ...
    public Node add(E e, P priority) {
        Node insertNode = new Node(e, priority, tree.size());
        tree.add(insertNode);
        pullUp(tree.size() - 1);
        return insertNode;
    }
     ...
     * the min heap is expected to be correct, and this is being called to
     * re-heapify
     ...
     * Node is based on a generic interface and takes the object types of 'E' and
     * 'P'
